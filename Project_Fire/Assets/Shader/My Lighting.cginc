...
float3 GetRamp(float nl)
{
	return tex2D(_RampTexture, float2(1 - nl,1 - nl));
}
...
UnityLight CreateLight (Interpolators i) {
	UnityLight light;
		...
		//更改
		if(attenuation <= 0)
			light.color = _LightColor0.rgb * _ShadowColor;
		else
...
#define UNITY_HANDLE_CORRECTLY_NEGATIVE_NDOTV 0
    ...
    half nl = saturate(dot(normal, light.dir) * 0.5 + 0.5);
	//更改
	nl = GetRamp(nl).r;
